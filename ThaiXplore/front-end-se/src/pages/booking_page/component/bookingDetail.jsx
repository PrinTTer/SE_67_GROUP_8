        ...
        <div className="flex flex-1 flex-col w-full h-full bg-white border-r-2 border-gray-200 rounded-lg shadow-sm">
            ...
            {/* Content section */}
            {
                Array.isArray(item.services)  ?
                        ...
                        <div className="overflow-y-auto h-screen hide-scrollbar">
                            {
                                item.services.map((element , index)=>(
                                    <ContentComponent key={index} Topic={Topic} quotation={item} item={element} formatFieldName={formatFieldName}/>
                                ))
                            }
                        </div>
                    )
                    :
                    (
                        <ContentComponent Topic={Topic} item={item} formatFieldName={formatFieldName}/>
                    )
            }
